...
df = pd.read_excel(file_path, sheet_name='Tech (Team)').fillna("N/A")
...
def send_email(to_email, details, qr_data):
    msg = EmailMessage()
    msg["Subject"] = f"🎉 Dakshaa Confirmation | {details['Preferred Technical Events']} Participation Approved 🎟️"
...
<body>
    <div class="container">
        ...
        <p>Thank you for registering your team <span class="highlight">{details['Team Name']}</span> for the <span class="highlight">{details['Preferred Technical Events']}</span> event!</p>
        <p><strong>Team Details:</strong></p>
        <ul>
            <li><strong>Team Name:</strong> {details['Team Name']}</li>
            <li><strong>Event Type:</strong> {details['Event Type']}</li>
            <li><strong>Preferred Event:</strong> {details['Preferred Technical Events']}</li>
...
for index, row in df.iterrows():
    ...
    if pd.notna(email):
        ...
        Event: {row['Preferred Technical Events']}
        """
