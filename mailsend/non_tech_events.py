...
df = pd.read_excel(file_path, sheet_name='Non-Tech Events')
...
def send_email(to_email, details, qr_data):
    msg = EmailMessage()
    msg["Subject"] = f"🎉  Dakshaa Confirmation: {details['Preferred Non-Technical Events']} Participation"
...
<body>
    <div class="container">
        ...
        <p>Thank you for registering for the <span class="highlight">{details['Preferred Non-Technical Events']}</span> Non - Technical Event!</p>
        ...
        <ul>
            ...
            <li><strong>Event Type :</strong> Non - Technical Event</li>

            <li><strong>Preferred Workshop:</strong> {details['Preferred Non-Technical Events']}</li>
        </ul>
...
for index, row in df.iterrows():
    ...
    if pd.notna(email):
        ...
        Preferred Non - Technical Event: {row['EVENTS LIST']}
        """
        ...
        details = {
            ...
            'Department': row['Department'],
            'Year': row['Year'],
            'Event Type': 'Non-Technical Events(solo)',
