...
df = pd.read_excel(file_path, sheet_name='Non-Tech(Team2)').fillna("N/A")
...
def send_email(to_email, details, qr_data):
    msg = EmailMessage()
    msg["Subject"] = f"🎉 Dakshaa Confirmation | {details['Preferred Non-Technical Events']} Participation Approved 🎟️"
...
<body>
    <div class="container">
        ...
        <p>Thank you for registering your team <span class="highlight">{details['Team Name']}</span> for the <span class="highlight">{details['Preferred Non-Technical Events']}</span> event!</p>
        <p><strong>Team Details:</strong></p>
        <ul>
            <li><strong>Team Name:</strong> {details['Team Name']}</li>
            <li><strong>Event Type:</strong> Non-Technical (2-members)</li>
            <li><strong>Event Name :</strong>{details['Preferred Non-Technical Events']}</li>
...
for index, row in df.iterrows():
    ...
    if pd.notna(email):
        ...
        Event Type: Non-Technical (2-members)
    ...
    'Year': row['Year'],
    'Event Type': 'Non-Technical (2-members)',
    'Preferred Non-Technical Events': row['EVENTS LIST'],
