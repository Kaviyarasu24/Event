...
df = pd.read_excel(file_path, sheet_name='project,paper,poster').fillna("N/A")
...
def send_email(to_email, details, qr_data):
    msg = EmailMessage()
    msg["Subject"] = f"🎉 Dakshaa Confirmation | {details['Preferred Presentation']} Participation Approved 🎟️"
...
<body>
    <div class="container">
        ...
        <p>Thank you for registering your team <span class="highlight">{details['Team Name']}</span> for the <span class="highlight">{details['Preferred Presentation']}</span> event!</p>
        <p><strong>Team Details:</strong></p>
        <ul>
            <li><strong>Team Name:</strong> {details['Team Name']}</li>
            <li><strong>Event Type: Paper/Project/Poster presentation</strong>  </li>
            <li><strong>Preferred Presentation:</strong> {details['Preferred Presentation']}</li>
...
for index, row in df.iterrows():
    ...
    if pd.notna(email):
        ...
        Event Type: Paper/Project/Poster presentation
        Preferred Presentation: {row['Preferred Event']}
        Mobile: {row['Team Lead Mobile Number']}
        Theme: {row['Themes']}
        Title: {row['Title of The Project/Paper/Poster']}
