...
df = pd.read_excel(file_path, sheet_name='workshop')
...
def send_email(to_email, details, qr_data):
    msg = EmailMessage()
    msg["Subject"] = f"🎉 Dakshaa Confirmation | {details['Preferred Workshop']} Participation Approved 🎟️"
...
<body>
    <div class="container">
        ...
        <p>Thank you for registering for the <span class="highlight">{details['Preferred Workshop']}</span> workshop!</p>
        ...
        <ul>
            ...
            <li><strong>Event Type:</strong> Workshop</li>
            <li><strong>Preferred Workshop:</strong> {details['Preferred Workshop']}</li>
        </ul>

        <div class="special-note">
    <p>🎉 <strong>Special Note!</strong> 🎉</p>
    <p>Good news! Every participant who has registered for a workshop is eligible for <strong>one free entry</strong> into a <strong>Non-Technical Event</strong>. 🏆</p>
    <p>📍 <strong>Registration for this non-tech event will be on the spot.</strong></p>
    <p>Don't miss this chance to participate and have fun!</p>
</div>

<div class="condition">
    <p><strong>⚠️ Condition Applicable </p>
</div>
...
"""
    ...
    try:
        with smtplib.SMTP_SSL(SMTP_SERVER, SMTP_PORT) as server:
            ...
            print(f"{emails_sent}✅ Email sent to {to_email}")
            return True
...
for index, row in df.iterrows():
    email = row['Email Address (Eg : abc@gmail.com)']
    if pd.notna(email):
        qr_data = f"""
        Name: {row['Name (Eg: Kumar T)']}
        Email: {row['Email Address (Eg : abc@gmail.com)']}
        Mobile: {row['Mobile Number (8675515313)']}
        College Name: {row['College Name (K.S.Rangasamy College of Technology)']}
        Stream of Study: {row['Stream of Study']}
        Department: {row['Department  (B.Tech (AI&DS)']}
        Year: {row['Year']}
        Event Type: Workshop
        Preferred Workshop: {row['Preferred Workshop']}
        """
        ...
        details = {
            'Name': row['Name (Eg: Kumar T)'],
            'Email': email,
            'Mobile Number': row['Mobile Number (8675515313)'],
            'College Name': row['College Name (K.S.Rangasamy College of Technology)'],
            'Stream of Study': row['Stream of Study'],
            'Department': row['Department  (B.Tech (AI&DS)'],
            'Year': row['Year'],
